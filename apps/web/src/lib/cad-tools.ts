...
export const CAD_TOOLS: Anthropic.Tool[] = [
  ...
  // === Routing (Pipe, Tube, Electrical) - Full Specification Support ===
  {
    name: "sw_create_pipe_route",
    description: "Create pipe route with full ASME/ISO specification. Supports all pipe standards, materials, schedules, and connection types.",
    input_schema: {
      type: "object" as const,
      properties: {
        ...
        pipe_standard: { type: "string", enum: ["ANSI", "ASME B31.1", "ASME B31.3", "ISO", "DIN", "JIS", "BS", "API", "ASTM"], description: "Pipe standard" },
        pipe_material: { type: "string", enum: ["carbon_steel", "stainless_steel_304", "stainless_steel_316", "copper", "pvc", "cpvc", "aluminum", "brass", "titanium", "inconel", "hastelloy", "duplex"], description: "Pipe material" },
        pipe_size: { type: "string", enum: ["1/8 inch", "1/4 inch", "3/8 inch", "1/2 inch", "3/4 inch", "1 inch", "1-1/4 inch", "1-1/2 inch", "2 inch", "2-1/2 inch", "3 inch", "4 inch", "5 inch", "6 inch", "8 inch", "10 inch", "12 inch", "14 inch", "16 inch", "18 inch", "20 inch", "24 inch", "DN6", "DN8", "DN10", "DN15", "DN20", "DN25", "DN32", "DN40", "DN50", "DN65", "DN80", "DN100", "DN125", "DN150", "DN200", "DN250", "DN300", "DN350", "DN400"], description: "Nominal pipe size (inch or DN metric)" },
        schedule: { type: "string", enum: ["5", "5S", "10", "10S", "20", "30", "40", "40S", "60", "80", "80S", "100", "120", "140", "160", "STD", "XS", "XXS"], description: "Pipe schedule/wall thickness" },
        bend_radius: { type: "number", description: "Bend radius in meters (default: 1.5D)" },
        connection_type: { type: "string", enum: ["butt_weld", "socket_weld", "threaded", "flanged", "grooved", "compression"], description: "Connection type" },
        insulation_thickness: { type: "number", description: "Insulation thickness in meters" },
      },
  ...
  },
  {
    ...
    description: "Insert routing fitting with full type library. Supports all elbow, tee, reducer, flange, and valve types with pressure ratings.",
    input_schema: {
      type: "object" as const,
      properties: {
        fitting_type: { type: "string", enum: ["elbow_90", "elbow_45", "elbow_long_radius", "elbow_short_radius", "elbow_3d", "tee_equal", "tee_reducing", "tee_lateral", "reducer_concentric", "reducer_eccentric", "flange_weld_neck", "flange_slip_on", "flange_blind", "flange_socket_weld", "flange_threaded", "flange_lap_joint", "valve_gate", "valve_globe", "valve_ball", "valve_butterfly", "valve_check", "valve_needle", "valve_plug", "cap", "coupling", "union", "cross", "wye", "swage", "stub_end", "olet_weld", "olet_thread", "olet_socket"], description: "Fitting type" },
        position: { type: "array", items: { type: "number" }, description: "Position [x, y, z] in meters" },
        size: { type: "string", description: "Primary fitting size" },
        size2: { type: "string", description: "Secondary size for reducers/reducing tees" },
        angle: { type: "number", description: "Angle for elbows (degrees)" },
        rating: { type: "string", enum: ["150", "300", "400", "600", "900", "1500", "2500", "2000", "3000", "6000"], description: "Pressure rating (class)" },
        facing: { type: "string", enum: ["raised_face", "flat_face", "ring_joint", "tongue_groove"], description: "Flange facing type" },
      },
  ...
  },
  {
    ...
    description: "Create electrical cable/wire route with full specification. Supports AWG, metric sizes, and communication cables.",
    input_schema: {
      type: "object" as const,
      properties: {
        ...
        cable_type: { type: "string", enum: ["10 AWG", "12 AWG", "14 AWG", "16 AWG", "18 AWG", "20 AWG", "22 AWG", "24 AWG", "0.5 mm2", "0.75 mm2", "1.0 mm2", "1.5 mm2", "2.5 mm2", "4.0 mm2", "6.0 mm2", "10 mm2", "CAT5e", "CAT6", "CAT6A", "fiber_single", "fiber_multi", "coax_rg6", "coax_rg59"], description: "Cable/wire type" },
        insulation: { type: "string", enum: ["PVC", "XLPE", "rubber", "silicone", "teflon", "polyethylene"], description: "Cable insulation type" },
        bundle_diameter: { type: "number", description: "Bundle diameter in meters" },
        min_bend_radius: { type: "number", description: "Minimum bend radius in meters" },
        shielded: { type: "boolean", description: "Shielded cable" },
        wire_count: { type: "number", description: "Number of wires in cable" },
      },
  ...
  },
  {
    ...
    description: "Add weld bead with full AWS D1.1/ISO 2553 specification. Supports all weld types, processes, and finishing options.",
    input_schema: {
      type: "object" as const,
      properties: {
        weld_type: { type: "string", enum: ["fillet", "groove_v", "groove_u", "groove_j", "groove_bevel", "groove_flare_v", "groove_flare_bevel", "plug", "slot", "spot", "seam", "stud", "surfacing", "edge", "corner"], description: "Weld type per AWS D1.1/ISO 2553" },
        face1: { type: "string", description: "First face to weld" },
        face2: { type: "string", description: "Second face to weld" },
        size: { type: "number", description: "Weld size in meters (leg size for fillet, depth for groove)" },
        throat: { type: "number", description: "Effective throat thickness in meters" },
        root_opening: { type: "number", description: "Root opening/gap in meters" },
        root_face: { type: "number", description: "Root face height in meters" },
        groove_angle: { type: "number", description: "Groove angle in degrees" },
        ...
        pitch: { type: "number", description: "Pitch/spacing between segments if intermittent" },
        stagger: { type: "boolean", description: "Stagger intermittent welds" },
        contour: { type: "string", enum: ["flat", "convex", "concave", "none"], description: "Weld contour/finish" },
        finishing: { type: "string", enum: ["grind", "machine", "chip", "none"], description: "Finishing method" },
        weld_process: { type: "string", enum: ["SMAW", "GMAW", "GTAW", "FCAW", "SAW", "RSW", "RSEW", "OFW", "PAW", "EBW", "LBW"], description: "Welding process" },
        filler_metal: { type: "string", description: "Filler metal specification (e.g., E70XX, ER70S-6)" },
      },
