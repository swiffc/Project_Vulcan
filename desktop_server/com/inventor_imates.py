...
class CreateIMatesRequest(BaseModel):
    ...
    part_ids: Optional[List[str]] = (
        None  # Specific part IDs to process (None = all parts)
    )
        ...
        try:
            ...
            for i in range(1, features.Count + 1):
                ...
                if hasattr(feature, "HoleType"):
                    ...
                    holes.append({"feature": feature, "name": f"Hole_{len(holes) + 1}"})
...
async def create_imates(req: CreateIMatesRequest):
    ...
    try:
        ...
        if doc_type == 12291:  # kAssemblyDocumentObject
            ...
            for hole in holes:
                if "edge" in hole:
                    ...
                    created_imates.append({"name": hole["name"], "type": "Insert"})
        ...
        raise HTTPException(
            status_code=500, detail=f"Failed to create iMates: {str(e)}"
        )
...
async def create_composite_imates(req: CreateCompositeIMatesRequest):
    ...
    try:
        ...
        if len(holes) < 2:
            raise HTTPException(
                status_code=400, detail="Need at least 2 holes for composite iMate"
            )
        ...
        for pattern_idx, pattern in enumerate(patterns):
            ...
            for hole in pattern:
                if "edge" in hole:
                    imate = create_insert_imate(comp_def, hole["edge"], hole["name"])
        ...
        raise HTTPException(
            status_code=500, detail=f"Failed to create composite iMates: {str(e)}"
        )
...
async def verify_alignment(req: VerifyAlignmentRequest):
    ...
    try:
        ...
        raise HTTPException(
            status_code=500, detail=f"Failed to verify alignment: {str(e)}"
        )
